...
# 获取一年前到当前的时间范围
def get_one_year_range():
    current = datetime.datetime.now()
    one_year_ago = current - datetime.timedelta(days=365)
    date_from = datetime.datetime(one_year_ago.year, one_year_ago.month, one_year_ago.day, 0, 0, 0)
    date_to = datetime.datetime(current.year, current.month, current.day, 23, 59, 59)
    return date_from, date_to
